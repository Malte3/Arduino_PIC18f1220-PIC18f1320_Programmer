...
static int reformat(HEX thehex,char *buffer){
	...
	if( iRecord == 4 ){
		...
		if( iAddress + (thehex->offset) < 0x2000 ){
			for(i=0;i<(iSize/2);i++){
				buffer += 2;
			...
			for(i=0;i<iSize/2;i++){
				...
				if((thehex->fuse[ iAddress + (thehex->offset) + i - 0x300000 ].value = hexStringToInt(buffer,4)) == -1) 
					return 1;
				thehex->fuse[ iAddress + (thehex->offset) + i - 0x300000 ].changed = 1;
				buffer += 4;
			}
		} else if (iAddress + (thehex->offset) >= 0x200000 && iAddress + (thehex->offset) <= 0x200007) {

			for (i = 0; i<(iSize / 2); i++) {
				buffer += 2;
				if ((thehex->ID[iAddress + i * 2 + (thehex->offset) - 0x200000] = hexStringToInt(buffer, 2)) == -1)
					return 1;
				buffer += 2;
				if ((thehex->ID[iAddress + i * 2 + (thehex->offset) - 0x200000] = hexStringToInt(buffer, 2)) == -1)
					return 1;
...
data getIDData(HEX thehex, unsigned int address) {
	return thehex->ID[address];
}
...
unsigned short getfuse(HEX thehex,data fuseID){
